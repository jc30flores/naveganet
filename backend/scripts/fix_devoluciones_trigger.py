#!/usr/bin/env python
"""Script para corregir el trigger de devoluciones"""
import os
import sys
import django
...
DECLARE
  ...
  v_cat_id BIGINT; v_cat_nombre TEXT;
BEGIN
  IF TG_OP='INSERT' OR NEW.producto_id IS DISTINCT FROM OLD.producto_id THEN
    ...
    NEW.producto_costo_snapshot  := NULL;
    NEW.producto_condicion_snapshot := NULL;
...
if __name__ == '__main__':
    print("Corrigiendo trigger devoluciones_snapshot_biu...")
    try:
        with connection.cursor() as cursor:
            cursor.execute(sql)
        print("OK: Trigger corregido exitosamente")
